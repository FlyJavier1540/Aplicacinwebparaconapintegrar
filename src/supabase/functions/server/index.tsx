...
app.get("/make-server-276018ed/health", (c) => {
 ...
 * 
 * @route POST /make-server-276018ed/init-data
 * @returns Resultado de la inicialización
 */
app.post("/make-server-276018ed/init-data", async (c) => {
  try {
 ...
 * 
 * @route GET /make-server-276018ed/check-init
 * @returns Estado de inicialización de datos base
 */
app.get("/make-server-276018ed/check-init", async (c) => {
  try {
    const supabase = createClient(
      Deno.env.get('SUPABASE_URL')!,
      Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!,
 ...
 * 
 * @route GET /make-server-276018ed/dashboard/stats
 * @returns Estadísticas del dashboard
 */
app.get("/make-server-276018ed/dashboard/stats", async (c) => {
  try {
 ...
 * 
 * @route GET /make-server-276018ed/dashboard/areas
 * @returns Array de áreas protegidas con coordenadas
 */
app.get("/make-server-276018ed/dashboard/areas", async (c) => {
  try {
    const accessToken = c.req.header('Authorization')?.split(' ')[1];
 ...
 * 
 * @route GET /make-server-276018ed/usuario/:email
 * @param email - Email del usuario autenticado
 * @returns Datos completos del usuario con relaciones (rol, área, estado)
 */
app.get("/make-server-276018ed/usuario/:email", async (c) => {
  try {
 ...
 * 
 * @route POST /make-server-276018ed/usuarios/:userId/cambiar-password
 * @param userId - ID del usuario al que se le cambiará la contraseña
 * @body { newPassword: string }
 * @returns { success: boolean }
 */
app.post("/make-server-276018ed/usuarios/:userId/cambiar-password", async (c) => {
  try {
 ...
 * 
 * @route GET /make-server-276018ed/usuarios
 * @returns Array de usuarios
 */
app.get("/make-server-276018ed/usuarios", async (c) => {
  try {
    const accessToken = c.req.header('Authorization')?.split(' ')[1];
 ...
 * 
 * @route POST /make-server-276018ed/usuarios
 * @body { nombre, apellido, cedula, telefono, email, password }
 * @returns Usuario creado
 */
app.post("/make-server-276018ed/usuarios", async (c) => {
  try {
    const accessToken = c.req.header('Authorization')?.split(' ')[1];
 ...
 * 
 * @route PUT /make-server-276018ed/usuarios/:userId
 * @body { nombre, apellido, telefono, email }
 * @returns Usuario actualizado
 */
app.put("/make-server-276018ed/usuarios/:userId", async (c) => {
  try {
    const userId = c.req.param('userId');
    const accessToken = c.req.header('Authorization')?.split(' ')[1];
 ...
 * 
 * @route PATCH /make-server-276018ed/usuarios/:userId/estado
 * @body { nuevoEstado: 'Activo' | 'Suspendido' | 'Desactivado' }
 * @returns Usuario actualizado
 */
app.patch("/make-server-276018ed/usuarios/:userId/estado", async (c) => {
  try {
    const userId = c.req.param('userId');
    const accessToken = c.req.header('Authorization')?.split(' ')[1];
 ...
 * 
 * @route GET /make-server-276018ed/guardarecursos
 * @returns Array de guardarecursos
 */
app.get("/make-server-276018ed/guardarecursos", async (c) => {
  try {
    const accessToken = c.req.header('Authorization')?.split(' ')[1];
 ...
 * 
 * @route POST /make-server-276018ed/guardarecursos
 * @body { nombre, apellido, dpi, telefono, email, password, areaAsignada }
 * @returns Guardarecurso creado
 */
app.post("/make-server-276018ed/guardarecursos", async (c) => {
  try {
    const accessToken = c.req.header('Authorization')?.split(' ')[1];
 ...
 * 
 * @route PUT /make-server-276018ed/guardarecursos/:id
 * @body { telefono, areaAsignada }
 * @returns Guardarecurso actualizado
 */
app.put("/make-server-276018ed/guardarecursos/:id", async (c) => {
  try {
 ...
 * 
 * @route PATCH /make-server-276018ed/guardarecursos/:id/estado
 * @body { nuevoEstado: 'Activo' | 'Suspendido' | 'Desactivado' }
 * @returns { success: boolean }
 */
app.patch("/make-server-276018ed/guardarecursos/:id/estado", async (c) => {
  try {
    const guardarecursoId = c.req.param('id');
    const accessToken = c.req.header('Authorization')?.split(' ')[1];
 ...
 * 
 * @route GET /make-server-276018ed/areas
 * @returns Array de áreas protegidas
 */
app.get("/make-server-276018ed/areas", async (c) => {
  try {
    const accessToken = c.req.header('Authorization')?.split(' ')[1];
...
app.post("/make-server-276018ed/areas", async (c) => {
  try {
    ...
    if (error) {
      console.error('Error al crear área protegida:', error);
      ...
      return c.json({ 
        success: false,
        error: "Error al crear área protegida" 
      }, 500);
    }
 ...
 * 
 * @route PUT /make-server-276018ed/areas/:id
 * @body { nombre, departamento, extension, fechaCreacion, lat, lng, descripcion, ecosistemas }
 * @returns Área protegida actualizada
 */
app.put("/make-server-276018ed/areas/:id", async (c) => {
  try {
 ...
 * 
 * @route PATCH /make-server-276018ed/areas/:id/estado
 * @body { nuevoEstado: 'Activo' | 'Desactivado' }
 * @returns { success: boolean }
 */
app.patch("/make-server-276018ed/areas/:id/estado", async (c) => {
  try {
    const areaId = c.req.param('id');
    const accessToken = c.req.header('Authorization')?.split(' ')[1];
...
app.get("/make-server-276018ed/guardarecursos", async (c) => {
  try {
    ...
    // Consultar guardarecursos con joins
    const { data, error } = await supabaseAdmin
      .from('usuario')
      .select(`
        usr_id,
    ...
    if (error) {
      ...
      return c.json({ 
        success: false,
        error: "Error al obtener guardarecursos" 
      }, 500);
    }
...
app.post("/make-server-276018ed/guardarecursos", async (c) => {
  try {
    ...
    if (areaAsignada) {
      const { data: areaData, error: errorArea } = await supabaseAdmin
        .from('area')
        .select('ar_id')
        .eq('ar_id', parseInt(areaAsignada))
        .maybeSingle();

      if (errorArea || !areaData) {
        return c.json({ 
          success: false,
          error: "El área protegida especificada no existe" 
        }, 400);
      }
    }
    ...
    // Insertar guardarecurso en la tabla usuario
    const { data: nuevoGuardarecurso, error } = await supabaseAdmin
      .from('usuario')
      .insert([{
        usr_nombre: nombre,
        ...
        usr_telefono: telefono || null,
        usr_correo: email,
        usr_area: areaAsignada ? parseInt(areaAsignada) : null,
        usr_rol: rolGuardarecurso.rl_id,
        usr_estado: estadoActivo.std_id
      }])
      .select(`
        usr_id,
    ...
    if (error) {
      console.error('Error al crear guardarecurso:', error);
      ...
      if (error.code === '23505') {
        return c.json({ 
          success: false,
          error: "Ya existe un guardarecurso con este email" 
        }, 400);
      }
      
      return c.json({ 
        success: false,
        error: "Error al crear guardarecurso en la base de datos" 
      }, 500);
    }
    ...
    return c.json({ 
      success: false,
      error: "Error interno del servidor al crear guardarecurso" 
    }, 500);
  }
});
...
app.put("/make-server-276018ed/guardarecursos/:id", async (c) => {
  try {
    ...
    // Verificar que el área existe si se especifica
    if (areaAsignada) {
      const { data: areaData, error: errorArea } = await supabaseAdmin
        .from('area')
        .select('ar_id')
 ...
 * 
 * @route GET /make-server-276018ed/equipos
 * @returns Array de equipos
 */
app.get("/make-server-276018ed/equipos", async (c) => {
  try {
    const accessToken = c.req.header('Authorization')?.split(' ')[1];
 ...
 * 
 * @route PUT /make-server-276018ed/equipos/:id
 * @body { observaciones, guardarecursoAsignado }
 * @returns Equipo actualizado
 */
app.put("/make-server-276018ed/equipos/:id", async (c) => {
  try {
 ...
 * 
 * @route PATCH /make-server-276018ed/equipos/:id/estado
 * @body { nuevoEstado: 'Operativo' | 'En Reparación' | 'Desactivado' }
 * @returns { success: boolean }
 */
app.patch("/make-server-276018ed/equipos/:id/estado", async (c) => {
  try {
    const equipoId = c.req.param('id');
    const accessToken = c.req.header('Authorization')?.split(' ')[1];
...
app.get("/make-server-276018ed/actividades", async (c) => {
  try {
    const accessToken = c.req.header('Authorization')?.split(' ')[1];
...
app.post("/make-server-276018ed/actividades", async (c) => {
  try {
    const accessToken = c.req.header('Authorization')?.split(' ')[1];
    if (!accessToken || accessToken === 'undefined') {
      return c.json({ success: false, error: "No autorizado" }, 401);
...
app.post("/make-server-276018ed/actividades/bulk", async (c) => {
  try {
    const accessToken = c.req.header('Authorization')?.split(' ')[1];
    if (!accessToken || accessToken === 'undefined') {
      return c.json({ success: false, error: "No autorizado" }, 401);
...
app.put("/make-server-276018ed/actividades/:id", async (c) => {
  try {
    const accessToken = c.req.header('Authorization')?.split(' ')[1];
    if (!accessToken || accessToken === 'undefined') {
      return c.json({ success: false, error: "No autorizado" }, 401);
...
app.delete("/make-server-276018ed/actividades/:id", async (c) => {
  try {
    const accessToken = c.req.header('Authorization')?.split(' ')[1];
    if (!accessToken || accessToken === 'undefined') {
      return c.json({ success: false, error: "No autorizado" }, 401);
 ...
 * @route GET /make-server-276018ed/actividades/patrullajes-en-progreso
 * @returns Patrullaje en progreso si existe
 */
app.get("/make-server-276018ed/actividades/patrullajes-en-progreso", async (c) => {
  try {
 ...
 * @route PUT /make-server-276018ed/actividades/:id/iniciar
 */
app.put("/make-server-276018ed/actividades/:id/iniciar", async (c) => {
  try {
    const accessToken = c.req.header('Authorization')?.split(' ')[1];
    if (!accessToken || accessToken === 'undefined') {
      return c.json({ success: false, error: "No autorizado" }, 401);
 ...
 * @route PUT /make-server-276018ed/actividades/:id/finalizar
 */
app.put("/make-server-276018ed/actividades/:id/finalizar", async (c) => {
  try {
    const accessToken = c.req.header('Authorization')?.split(' ')[1];
    if (!accessToken || accessToken === 'undefined') {
      return c.json({ success: false, error: "No autorizado" }, 401);
 ...
 * @route POST /make-server-276018ed/actividades/:id/coordenadas
 ...
 */
app.post('/make-server-276018ed/actividades/:id/coordenadas', async (c) => {
  try {
 ...
 * @route DELETE /make-server-276018ed/actividades/:id/coordenadas/:coordId
 * @param id - ID de la actividad
 * @param coordId - ID de la coordenada
 * @returns Confirmación de eliminación
 */
app.delete('/make-server-276018ed/actividades/:id/coordenadas/:coordId', async (c) => {
  try {
    const accessToken = c.req.header('Authorization')?.replace('Bearer ', '');
    if (!accessToken || accessToken === 'undefined') {
      return c.json({ success: false, error: "No autorizado" }, 401);
 ...
 * @route POST /make-server-276018ed/actividades/:id/hallazgos
 * @param id - ID de la actividad
 * @body { titulo, descripcion, gravedad, latitud, longitud }
 * @returns Hallazgo creado
 */
app.post('/make-server-276018ed/actividades/:id/hallazgos', async (c) => {
  try {
    const accessToken = c.req.header('Authorization')?.replace('Bearer ', '');
    if (!accessToken || accessToken === 'undefined') {
      return c.json({ success: false, error: "No autorizado" }, 401);
...
/**
 * Eliminar hallazgo
 * @route DELETE /make-server-276018ed/actividades/:id/hallazgos/:hallazgoId
 * @param id - ID de la actividad
 * @param hallazgoId - ID del hallazgo
 * @returns Confirmación de eliminación
 */
app.delete('/make-server-276018ed/actividades/:id/hallazgos/:hallazgoId', async (c) => {
  try {
    const accessToken = c.req.header('Authorization')?.replace('Bearer ', '');
    if (!accessToken || accessToken === 'undefined') {
      return c.json({ success: false, error: "No autorizado" }, 401);
 ...
 * @route GET /make-server-276018ed/rutas
 ...
 */
app.get("/make-server-276018ed/rutas", async (c) => {
  try {
    const accessToken = c.req.header('Authorization')?.split(' ')[1];
    if (!accessToken || accessToken === 'undefined') {
      return c.json({ success: false, error: "No autorizado" }, 401);
 ...
 * @route GET /make-server-276018ed/incidentes
 * @returns Array de incidentes
 */
app.get("/make-server-276018ed/incidentes", async (c) => {
  try {
    const accessToken = c.req.header('Authorization')?.split(' ')[1];
    if (!accessToken || accessToken === 'undefined') {
      return c.json({ success: false, error: "No autorizado" }, 401);
 ...
 * @route POST /make-server-276018ed/incidentes
 * @body { titulo, descripcion, categoria }
 * @returns Incidente creado
 */
app.post("/make-server-276018ed/incidentes", async (c) => {
  try {
    const accessToken = c.req.header('Authorization')?.split(' ')[1];
    if (!accessToken || accessToken === 'undefined') {
      return c.json({ success: false, error: "No autorizado" }, 401);
 ...
 * @route PATCH /make-server-276018ed/incidentes/:id/estado
 * @body { nuevoEstado }
 * @returns Incidente actualizado
 */
app.patch("/make-server-276018ed/incidentes/:id/estado", async (c) => {
  try {
    const accessToken = c.req.header('Authorization')?.split(' ')[1];
    if (!accessToken || accessToken === 'undefined') {
      return c.json({ success: false, error: "No autorizado" }, 401);
 ...
 * @route DELETE /make-server-276018ed/incidentes/:id
 * @returns Mensaje de éxito
 */
app.delete("/make-server-276018ed/incidentes/:id", async (c) => {
  try {
    const accessToken = c.req.header('Authorization')?.split(' ')[1];
    if (!accessToken || accessToken === 'undefined') {
      return c.json({ success: false, error: "No autorizado" }, 401);
 ...
 * @route POST /make-server-276018ed/incidentes/:id/seguimiento
 * @body { accion, observaciones }
 * @returns Seguimiento creado
 */
app.post("/make-server-276018ed/incidentes/:id/seguimiento", async (c) => {
  try {
    const accessToken = c.req.header('Authorization')?.split(' ')[1];
    if (!accessToken || accessToken === 'undefined') {
      return c.json({ success: false, error: "No autorizado" }, 401);
 ...
 * @route GET /make-server-276018ed/hallazgos
 * @returns Array de hallazgos
 */
app.get("/make-server-276018ed/hallazgos", async (c) => {
  try {
    const accessToken = c.req.header('Authorization')?.split(' ')[1];
    if (!accessToken || accessToken === 'undefined') {
      return c.json({ success: false, error: "No autorizado" }, 401);
 ...
 * @route POST /make-server-276018ed/hallazgos
 * @body { titulo, descripcion, ubicacion, prioridad, coordenadas, observaciones }
 * @returns Hallazgo creado
 */
app.post("/make-server-276018ed/hallazgos", async (c) => {
  try {
    const accessToken = c.req.header('Authorization')?.split(' ')[1];
    if (!accessToken || accessToken === 'undefined') {
      return c.json({ success: false, error: "No autorizado" }, 401);
 ...
 * @route PATCH /make-server-276018ed/hallazgos/:id/estado
 * @body { nuevoEstado }
 * @returns Hallazgo actualizado
 */
app.patch("/make-server-276018ed/hallazgos/:id/estado", async (c) => {
  try {
    const accessToken = c.req.header('Authorization')?.split(' ')[1];
    if (!accessToken || accessToken === 'undefined') {
      return c.json({ success: false, error: "No autorizado" }, 401);
 ...
 * @route DELETE /make-server-276018ed/hallazgos/:id
 * @returns Mensaje de éxito
 */
app.delete("/make-server-276018ed/hallazgos/:id", async (c) => {
  try {
    const accessToken = c.req.header('Authorization')?.split(' ')[1];
    if (!accessToken || accessToken === 'undefined') {
      return c.json({ success: false, error: "No autorizado" }, 401);
 ...
 * @route POST /make-server-276018ed/hallazgos/:id/seguimiento
 * @body { accion, observaciones }
 * @returns Seguimiento creado
 */
app.post("/make-server-276018ed/hallazgos/:id/seguimiento", async (c) => {
  try {
    const accessToken = c.req.header('Authorization')?.split(' ')[1];
    if (!accessToken || accessToken === 'undefined') {
      return c.json({ success: false, error: "No autorizado" }, 401);
 ...
 * @route GET /make-server-276018ed/metricas-cumplimiento
 * @query periodo (Mensual, Trimestral, Anual)
 * @query guardarecurso (opcional, ID del guardarecurso)
 * @returns Métricas de cumplimiento calculadas
 */
app.get("/make-server-276018ed/metricas-cumplimiento", async (c) => {
  try {
    const accessToken = c.req.header('Authorization')?.split(' ')[1];
    if (!accessToken || accessToken === 'undefined') {
      return c.json({ success: false, error: "No autorizado" }, 401);
    ...
    const { data: usuarioData, error: usuarioError } = await supabaseAdmin
      .from('usuario')
      .select('usr_id, usr_rol, rol:usr_rol(rl_nombre)')
      .eq('usr_correo', userData.user.email)
      .maybeSingle();
    ...
    const userRole = usuarioData.rol?.rl_nombre;
    const isGuardarecurso = userRole === 'Guardarecurso';

    // Obtener parámetros de consulta
    const url = new URL(c.req.url);
    const periodo = url.searchParams.get('periodo') || 'Mensual';
    const guardarecursoId = url.searchParams.get('guardarecurso');
    ...
    // Determinar qué usuarios incluir
    let usuariosFilter: any = supabaseAdmin
      .from('usuario')
      .select('usr_id, usr_nombre, usr_apellido')
      .eq('usr_rol', 3); // Rol Guardarecurso

    if (isGuardarecurso) {
      // Si es guardarecurso, solo sus datos
      usuariosFilter = usuariosFilter.eq('usr_id', usuarioData.usr_id);
    } else if (guardarecursoId && guardarecursoId !== 'todos') {
      // Si es admin/coordinador y seleccionó un guardarecurso específico
      usuariosFilter = usuariosFilter.eq('usr_id', parseInt(guardarecursoId));
    }

    const { data: usuarios, error: usuariosError } = await usuariosFilter;
    ...
    console.log(`📅 Período: ${periodo} | Desde: ${fechaInicio.toISOString()} | Hasta: ${fechaFin.toISOString()}`);

    for (const usuario of usuarios || []) {
      console.log(`\n🔍 Procesando guardarecurso: ${usuario.usr_nombre} ${usuario.usr_apellido} (ID: ${usuario.usr_id})`);

      // 1. MÉTRICA: Actividades Completas/Total Actividades = Porcentaje
      const { data: todasActividades, error: errorActividades } = await supabaseAdmin
        .from('actividad')
        .select('act_id, estado:act_estado(std_nombre)')
        .eq('act_usuario', usuario.usr_id)
        .gte('act_fechah_programacion', fechaInicio.toISOString())
        .lte('act_fechah_programacion', fechaFin.toISOString());

      if (errorActividades) {
        console.error(`❌ Error al consultar actividades del usuario ${usuario.usr_id}:`, errorActividades);
      }

      const totalActividades = todasActividades?.length || 0;
      const actividadesCompletadas = todasActividades?.filter(
        (a: any) => a.estado?.std_nombre === 'Completada'
      ).length || 0;
      ...
      const porcentajeActividades = totalActividades > 0 
        ? Math.round((actividadesCompletadas / totalActividades) * 100)
        : 0;

      metricas.push({
        id: `act-${usuario.usr_id}`,
        nombre: 'Cumplimiento de Actividades',
        descripcion: `${actividadesCompletadas} de ${totalActividades} actividades completadas`,
        tipo: 'Actividades',
        meta: 100,
        actual: porcentajeActividades,
        unidad: '%',
        periodo: periodo,
        guardarecurso: usuario.usr_id.toString(),
        guardarecursoNombre: `${usuario.usr_nombre} ${usuario.usr_apellido}`,
        fechaInicio: fechaInicio.toISOString(),
        fechaFin: fechaFin.toISOString()
      });

      // 2. MÉTRICA: Patrullajes Completos/Total Patrullajes = Porcentaje
      // Obtener actividades de tipo "Patrullaje"
      const { data: todosPatrullajes, error: errorPatrullajes } = await supabaseAdmin
        .from('actividad')
        .select('act_id, estado:act_estado(std_nombre), tipo:act_tipo_actividad(tpact_nombre)')
        .eq('act_usuario', usuario.usr_id)
        .gte('act_fechah_programacion', fechaInicio.toISOString())
        .lte('act_fechah_programacion', fechaFin.toISOString());

      if (errorPatrullajes) {
        console.error(`❌ Error al consultar patrullajes del usuario ${usuario.usr_id}:`, errorPatrullajes);
      }

      // Filtrar solo patrullajes
      const patrullajes = todosPatrullajes?.filter(
        (a: any) => a.tipo?.tpact_nombre === 'Patrullaje'
      ) || [];

      const totalPatrullajes = patrullajes.length;
      const patrullajesCompletados = patrullajes.filter(
        (a: any) => a.estado?.std_nombre === 'Completada'
      ).length;

      console.log(`  🚶 Patrullajes: ${patrullajesCompletados}/${totalPatrullajes} completados`);

      // Calcular porcentaje de patrullajes completados
      const porcentajePatrullajes = totalPatrullajes > 0 
        ? Math.round((patrullajesCompletados / totalPatrullajes) * 100)
        : 0;

      metricas.push({
        id: `pat-${usuario.usr_id}`,
        nombre: 'Cumplimiento de Patrullajes',
        descripcion: `${patrullajesCompletados} de ${totalPatrullajes} patrullajes completados`,
        tipo: 'Patrullajes',
        meta: 100,
        actual: porcentajePatrullajes,
        unidad: '%',
        periodo: periodo,
        guardarecurso: usuario.usr_id.toString(),
        guardarecursoNombre: `${usuario.usr_nombre} ${usuario.usr_apellido}`,
      ...
      // 3. MÉTRICA: Hallazgos = Cantidad
      const { data: hallazgosReportados, error: errorHallazgos } = await supabaseAdmin
        .from('hallazgo')
        .select('hlz_id')
        .eq('hlz_usuario', usuario.usr_id)
        .gte('hlz_fecha', fechaInicio.toISOString().split('T')[0])
        .lte('hlz_fecha', fechaFin.toISOString().split('T')[0]);

      if (errorHallazgos) {
        console.error(`❌ Error al consultar hallazgos del usuario ${usuario.usr_id}:`, errorHallazgos);
      }

      const cantidadHallazgos = hallazgosReportados?.length || 0;
      console.log(`  📍 Hallazgos: ${cantidadHallazgos} reportados`);

      metricas.push({
        id: `hal-${usuario.usr_id}`,
        nombre: 'Hallazgos Reportados',
        descripcion: `${cantidadHallazgos} hallazgos reportados en el período`,
        tipo: 'Hallazgos',
        meta: cantidadHallazgos > 0 ? cantidadHallazgos : 1, // Meta = cantidad real para mostrar 100%
        actual: cantidadHallazgos,
        unidad: 'hallazgos',
        periodo: periodo,
        guardarecurso: usuario.usr_id.toString(),
        guardarecursoNombre: `${usuario.usr_nombre} ${usuario.usr_apellido}`,
    ...
    console.log(`\n✅ Total de métricas generadas: ${metricas.length}`);
    return c.json({ success: true, metricas });
  } catch (error) {
    console.error('Error en GET /metricas-cumplimiento:', error);
    return c.json({ success: false, error: "Error interno del servidor" }, 500);
  }
});
...
/**
 * =============================================
 * REGISTRO FOTOGRÁFICO - ENDPOINTS
 * =============================================
 */

/**
 * Obtener todas las evidencias fotográficas
 * @route GET /make-server-276018ed/fotografias
 * @returns Lista de fotografías
 */
app.get("/make-server-276018ed/fotografias", async (c) => {
  try {
    const accessToken = c.req.header('Authorization')?.split(' ')[1];
    if (!accessToken || accessToken === 'undefined') {
      return c.json({ success: false, error: "No autorizado" }, 401);
    ...
    let query = supabaseAdmin
      ...
      .select(`
        ...
        ftg_actividad,
        ftg_usuario,
        usuario:ftg_usuario(usr_id, usr_nombre, usr_apellido),
        actividad:ftg_actividad(act_id, act_descripcion, act_tipo_actividad)
      `)
      .order('ftg_fecha', { ascending: false });

    // Si es guardarecurso, solo ver sus fotografías
    if (isGuardarecurso) {
      query = query.eq('ftg_usuario', usuarioData.usr_id);
    }

    const { data: fotografias, error: fotografiasError } = await query;

    if (fotografiasError) {
      console.error('Error al obtener fotografías:', fotografiasError);
      return c.json({ success: false, error: "Error al obtener fotografías" }, 500);
    }

    // Transformar datos al formato esperado por el frontend
    const fotografiasFormateadas = fotografias?.map((foto: any) => ({
      id: foto.ftg_id.toString(),
      url: foto.ftg_url,
      descripcion: foto.ftg_descripcion,
      coordenadas: {
        lat: foto.ftg_coordenadas_lat,
        lng: foto.ftg_coordenadas_lng
      },
      fecha: foto.ftg_fecha,
      tipo: foto.ftg_tipo,
      observaciones: foto.ftg_observaciones,
      actividad: foto.ftg_actividad?.toString(),
      guardarecurso: foto.ftg_usuario?.toString(),
      guardarecursoNombre: foto.usuario 
        ? `${foto.usuario.usr_nombre} ${foto.usuario.usr_apellido}`
        : 'Desconocido',
      actividadDescripcion: foto.actividad?.act_descripcion
    })) || [];

    return c.json({ success: true, fotografias: fotografiasFormateadas });
  } catch (error) {
    console.error('Error en GET /fotografias:', error);
    return c.json({ success: false, error: "Error interno del servidor" }, 500);
  }
});
 ...
 * @route POST /make-server-276018ed/fotografias
 * @body { descripcion, coordenadas_lat, coordenadas_lng, tipo, actividad, observaciones, url }
 * @returns Fotografía creada
 */
app.post("/make-server-276018ed/fotografias", async (c) => {
  try {
    const accessToken = c.req.header('Authorization')?.split(' ')[1];
    if (!accessToken || accessToken === 'undefined') {
      return c.json({ success: false, error: "No autorizado" }, 401);
    ...
    // Obtener datos del body
    const body = await c.req.json();
    const { descripcion, coordenadas_lat, coordenadas_lng, tipo, actividad, observaciones, url } = body;

    // Validaciones
    if (!descripcion || descripcion.trim() === '') {
      return c.json({ success: false, error: "La descripción es requerida" }, 400);
    }

    if (!url || url.trim() === '') {
      return c.json({ success: false, error: "La URL de la imagen es requerida" }, 400);
    }

    // Insertar fotografía con fecha/hora de Guatemala
    const { data: nuevaFotografia, error: insertError } = await supabaseAdmin
      .from('fotografia')
      .insert({
        ftg_descripcion: descripcion,
        ftg_coordenadas_lat: coordenadas_lat || 0,
        ftg_coordenadas_lng: coordenadas_lng || 0,
        ftg_tipo: tipo || 'Otro',
        ftg_actividad: actividad ? parseInt(actividad) : null,
        ftg_observaciones: observaciones || '',
        ftg_url: url,
        ftg_usuario: usuarioData.usr_id,
        ftg_fecha: getGuatemalaDateTime()
      })
      .select()
      .single();

    if (insertError) {
      console.error('Error al crear fotografía:', insertError);
      return c.json({ success: false, error: "Error al crear fotografía" }, 500);
    }

    return c.json({ 
      success: true, 
      fotografia: {
        id: nuevaFotografia.ftg_id.toString(),
        url: nuevaFotografia.ftg_url,
        descripcion: nuevaFotografia.ftg_descripcion,
        coordenadas: {
          lat: nuevaFotografia.ftg_coordenadas_lat,
          lng: nuevaFotografia.ftg_coordenadas_lng
        },
        fecha: nuevaFotografia.ftg_fecha,
        tipo: nuevaFotografia.ftg_tipo,
        observaciones: nuevaFotografia.ftg_observaciones,
        actividad: nuevaFotografia.ftg_actividad?.toString(),
        guardarecurso: nuevaFotografia.ftg_usuario?.toString()
      }
    });
  } catch (error) {
    console.error('Error en POST /fotografias:', error);
    return c.json({ success: false, error: "Error interno del servidor" }, 500);
  }
});
...
/**
 * Eliminar fotografía
 * @route DELETE /make-server-276018ed/fotografias/:id
 * @returns Confirmación de eliminación
 */
app.delete("/make-server-276018ed/fotografias/:id", async (c) => {
  try {
    const accessToken = c.req.header('Authorization')?.split(' ')[1];
    if (!accessToken || accessToken === 'undefined') {
      return c.json({ success: false, error: "No autorizado" }, 401);
    ...
    const fotografiaId = c.req.param('id');
    const userRole = usuarioData.rol?.rl_nombre;

    // Verificar que la fotografía existe
    const { data: fotografia, error: fotoError } = await supabaseAdmin
      .from('fotografia')
      .select('ftg_id, ftg_usuario')
      .eq('ftg_id', parseInt(fotografiaId))
      .maybeSingle();

    if (fotoError || !fotografia) {
      return c.json({ success: false, error: "Fotografía no encontrada" }, 404);
    }

    // Validar permisos: solo el creador o admin/coordinador pueden eliminar
    const isOwner = fotografia.ftg_usuario === usuarioData.usr_id;
    const isAdminOrCoord = userRole === 'Administrador' || userRole === 'Coordinador';

    if (!isOwner && !isAdminOrCoord) {
      return c.json({ success: false, error: "No tienes permisos para eliminar esta fotografía" }, 403);
    }
    ...
    if (deleteError) {
      console.error('Error al eliminar fotografía:', deleteError);
      return c.json({ success: false, error: "Error al eliminar fotografía" }, 500);
    }

    return c.json({ success: true, message: "Fotografía eliminada correctamente" });
  } catch (error) {
    console.error('Error en DELETE /fotografias/:id:', error);
    return c.json({ success: false, error: "Error interno del servidor" }, 500);
  }
});
 ...
 * @route GET /make-server-276018ed/debug/usuario/:id
 */
app.get("/make-server-276018ed/debug/usuario/:id", async (c) => {
  try {
    ...
    let query = supabaseAdmin
      ...
      .select(`
        *,
        ...
        estado_rel:hlz_estado(std_id, std_nombre)
      `)
      .order('hlz_fecha', { ascending: false });

    if (usuarioData.rol?.rl_nombre === 'Guardarecurso') {
      query = query.eq('hlz_usuario', usuarioData.usr_id);
    }

    const { data: hallazgos, error: hallazgosError } = await query;

    if (hallazgosError) {
      console.error('Error al obtener hallazgos:', hallazgosError);
      return c.json({ success: false, error: "Error al obtener hallazgos" }, 500);
    }

    console.log('Hallazgos obtenidos de Supabase:', hallazgos?.length || 0);
    if (hallazgos && hallazgos.length > 0) {
      console.log('Primer hallazgo raw:', JSON.stringify(hallazgos[0], null, 2));
    }

    // Obtener seguimientos para cada hallazgo
    const hallazgosConSeguimiento = await Promise.all(
      (hallazgos || []).map(async (hlz: any) => {
        const { data: seguimientos } = await supabaseAdmin
          ...
          .select(`
            ...
            sgm_observaciones,
            sgm_fecha,
            usuario:sgm_usuario(usr_nombre, usr_apellido)
          `)
          .eq('sgm_hallazgo', hlz.hlz_id)
          .order('sgm_fecha', { ascending: false });

        const seguimientoFormateado = seguimientos?.map((seg: any) => ({
          fecha: seg.sgm_fecha,
          accion: seg.sgm_accion,
          responsable: seg.usuario ? `${seg.usuario.usr_nombre} ${seg.usuario.usr_apellido}` : 'Sistema',
          observaciones: seg.sgm_observaciones
        })) || [];

        return { ...hlz, seguimientos: seguimientoFormateado };
      })
    );

    // Transformar datos al formato esperado por el frontend
    const hallazgosFormateados = hallazgosConSeguimiento.map((hlz: any) => {
      // Debug: Log para ver la estructura de datos
      console.log('Hallazgo completo:', JSON.stringify({
        hlz_id: hlz.hlz_id,
        hlz_usuario: hlz.hlz_usuario,
        usuario: hlz.usuario ? {
          usr_id: hlz.usuario.usr_id,
          usr_nombre: hlz.usuario.usr_nombre,
          usr_apellido: hlz.usuario.usr_apellido,
          usr_area: hlz.usuario.usr_area,
          area: hlz.usuario.area
        } : null
      }, null, 2));

      return {
        id: hlz.hlz_id.toString(),
        titulo: hlz.hlz_nombre,
        descripcion: hlz.hlz_descripcion,
        prioridad: hlz.categoria?.ctg_nombre || 'Media', // Mapear categoría a prioridad
        estado: hlz.estado_rel?.std_nombre || 'Reportado',
        ubicacion: `${hlz.hlz_latitud.toFixed(6)}, ${hlz.hlz_longitud.toFixed(6)}`,
        coordenadas: {
          lat: hlz.hlz_latitud,
          lng: hlz.hlz_longitud
        },
        areaProtegida: hlz.usuario?.area?.ar_id?.toString() || '',
        areaProtegidaNombre: hlz.usuario?.area?.ar_nombre || 'N/A',
        guardarecurso: hlz.hlz_usuario?.toString() || '',
        guardarecursoNombre: hlz.usuario 
          ? `${hlz.usuario.usr_nombre} ${hlz.usuario.usr_apellido}`
          : 'Desconocido',
        fechaReporte: hlz.hlz_fecha,
        observaciones: '',
        evidencias: [],
        seguimiento: hlz.seguimientos
      };
    }) || [];

    return c.json({ success: true, hallazgos: hallazgosFormateados });
  } catch (error) {
    console.error('Error en GET /hallazgos:', error);
    return c.json({ success: false, error: "Error interno del servidor" }, 500);
  }
 ...
 * @route POST /make-server-276018ed/hallazgos
 * @body { titulo, descripcion, prioridad, ubicacion, coordenadas, areaProtegida, observaciones }
 * @returns Hallazgo creado
 */
app.post("/make-server-276018ed/hallazgos", async (c) => {
  try {
    ...
    const body = await c.req.json();
    const { titulo, descripcion, prioridad, ubicacion, coordenadas } = body;

    // Obtener o crear la categoría basada en la prioridad
    const { data: categoriaData } = await supabaseAdmin
      .from('categoria')
      .select('ctg_id')
      .eq('ctg_nombre', prioridad)
      .maybeSingle();

    let categoriaId = categoriaData?.ctg_id;

    // Si no existe la categoría, crearla
    if (!categoriaId) {
      const { data: nuevaCategoria } = await supabaseAdmin
        .from('categoria')
        .insert([{ ctg_nombre: prioridad }])
        .select('ctg_id')
        .single();
      categoriaId = nuevaCategoria?.ctg_id;
    }
    ...
    if (!estadoData) {
      return c.json({ success: false, error: "Estado 'Reportado' no encontrado" }, 400);
    }

    // Crear hallazgo
    const { data: nuevoHallazgo, error: createError } = await supabaseAdmin
      .from('hallazgo')
      .insert([{
        hlz_nombre: titulo,
        hlz_descripcion: descripcion,
        hlz_categoria: categoriaId,
        hlz_latitud: coordenadas?.lat || 0,
        hlz_longitud: coordenadas?.lng || 0,
        hlz_fecha: new Date().toISOString().split('T')[0],
        hlz_usuario: usuarioData.usr_id,
        hlz_estado: estadoData.std_id
      }])
      .select(`
    ...
    if (createError) {
      console.error('Error al crear hallazgo:', createError);
      return c.json({ success: false, error: "Error al crear hallazgo" }, 500);
    }
    ...
    return c.json({ 
      ...
      hallazgo: {
        ...
        titulo: nuevoHallazgo.hlz_nombre,
        descripcion: nuevoHallazgo.hlz_descripcion,
        prioridad: nuevoHallazgo.categoria?.ctg_nombre || prioridad,
        estado: nuevoHallazgo.estado_rel?.std_nombre || 'Reportado',
        ubicacion: `${nuevoHallazgo.hlz_latitud.toFixed(6)}, ${nuevoHallazgo.hlz_longitud.toFixed(6)}`,
        coordenadas: {
          lat: nuevoHallazgo.hlz_latitud,
          lng: nuevoHallazgo.hlz_longitud
        ...
        areaProtegida: usuarioData.area?.ar_id?.toString() || '',
        areaProtegidaNombre: usuarioData.area?.ar_nombre || 'N/A',
        guardarecurso: usuarioData.usr_id.toString(),
        guardarecursoNombre: `${usuarioData.usr_nombre} ${usuarioData.usr_apellido}`,
        fechaReporte: nuevoHallazgo.hlz_fecha,
        observaciones: '',
        evidencias: [],
        seguimiento: []
...
app.patch("/make-server-276018ed/hallazgos/:id/estado", async (c) => {
  try {
    ...
    // Obtener hallazgo actual
    const { data: hallazgoActual } = await supabaseAdmin
      .from('hallazgo')
      .select(`
        *,
        estado_rel:hlz_estado(std_nombre)
      `)
      .eq('hlz_id', parseInt(hallazgoId))
      .single();

    if (!hallazgoActual) {
      return c.json({ success: false, error: "Hallazgo no encontrado" }, 404);
    }

    // Obtener el nuevo estado
    const { data: nuevoEstadoData } = await supabaseAdmin
      .from('estado')
      .select('std_id')
      .eq('std_nombre', nuevoEstado)
      .maybeSingle();
    ...
    await supabaseAdmin
      .from('seguimiento')
      .insert([{
        sgm_accion: `Cambio de estado a: ${nuevoEstado}`,
        sgm_observaciones: `El hallazgo cambió de estado de "${hallazgoActual.estado_rel?.std_nombre || 'Desconocido'}" a "${nuevoEstado}"`,
        sgm_fecha: new Date().toISOString().split('T')[0],
        sgm_usuario: usuarioData.usr_id,
        sgm_hallazgo: parseInt(hallazgoId)
      }]);

    // Actualizar hallazgo
    const { data: hallazgoActualizado, error: updateError } = await supabaseAdmin
      .from('hallazgo')
      .update({
        hlz_estado: nuevoEstadoData.std_id
      })
      .eq('hlz_id', parseInt(hallazgoId))
      .select(`
        *,
    ...
    if (updateError) {
      ...
      return c.json({ success: false, error: "Error al actualizar estado" }, 500);
    }
    ...
    const { data: seguimientos } = await supabaseAdmin
      ...
      .select(`
        ...
        usuario:sgm_usuario(usr_nombre, usr_apellido)
      `)
      .eq('sgm_hallazgo', parseInt(hallazgoId))
      .order('sgm_fecha', { ascending: false });
    ...
    return c.json({ 
      success: true, 
      hallazgo: {
        id: hallazgoActualizado.hlz_id.toString(),
        titulo: hallazgoActualizado.hlz_nombre,
        descripcion: hallazgoActualizado.hlz_descripcion,
        prioridad: hallazgoActualizado.categoria?.ctg_nombre || 'Media',
        estado: hallazgoActualizado.estado_rel?.std_nombre || 'Reportado',
        ubicacion: `${hallazgoActualizado.hlz_latitud.toFixed(6)}, ${hallazgoActualizado.hlz_longitud.toFixed(6)}`,
        coordenadas: {
          lat: hallazgoActualizado.hlz_latitud,
          lng: hallazgoActualizado.hlz_longitud
        },
        areaProtegida: hallazgoActualizado.usuario?.area?.ar_id?.toString() || '',
        areaProtegidaNombre: hallazgoActualizado.usuario?.area?.ar_nombre || 'N/A',
        guardarecurso: hallazgoActualizado.hlz_usuario?.toString() || '',
        guardarecursoNombre: hallazgoActualizado.usuario 
          ? `${hallazgoActualizado.usuario.usr_nombre} ${hallazgoActualizado.usuario.usr_apellido}`
          : 'Desconocido',
        fechaReporte: hallazgoActualizado.hlz_fecha,
        observaciones: '',
        evidencias: [],
        seguimiento: seguimientoFormateado
      }
    });
  } catch (error) {
    console.error('Error en PATCH /hallazgos/:id/estado:', error);
 ...
 * @route POST /make-server-276018ed/cambiar-contrasena
 */
app.post("/make-server-276018ed/cambiar-contrasena", async (c) => {
  try {
    const accessToken = c.req.header('Authorization')?.split(' ')[1];
    if (!accessToken || accessToken === 'undefined') {
      return c.json({ success: false, error: "No autorizado" }, 401);
